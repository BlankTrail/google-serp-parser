...
var catalogue = map[Lang]map[string]string{
	LangEN: {
		...
		"proxies.resting":        "Resting off a failure",
		"proxies.ports":          "Ports open",
		"proxies.warm":           "Of them warm",
		"proxies.quarantined":    "Ports in quarantine",
		"proxies.pool.why":       "What is true this second, not a count of what happened. An address that failed goes to rest for a while and comes back; a run whose resting count climbs towards its total is about to have nothing left to try.",
		...
		"proxies.kind.transport": "Never arrived — connection refused, reset or closed",
		"proxies.kind.port":      "The proxy's own port did not answer — nothing went through it",
		"proxies.kind.relay":     "Relay refused — the address terminates TLS itself",
		"proxies.kind.wall":      "Wall — the origin refused this identity",
		"proxies.kind.timeout":   "Our own clock ran out",
		"proxies.kind.other":     "Anything else",
		"proxies.reset":          "Clear the counts",
		"proxies.release":        "Let the bench go",
		"proxies.reset.why":      "Puts every count above back to nought so a reading can be started from here. It clears counts and not state: an address resting off a failure goes on resting, and a port in quarantine stays in quarantine.",
		"proxies.release.why":    "Takes every resting address back into rotation and forgets the rests written down. It is for a bench filled by something that was never the addresses doing — the proxy service restarting, a network away for a minute — where every entry on it is evidence of one event rather than of a whole list. The counts are left alone.",
		...
		"form.kind.why":          "What each type does with a line of the list:",
		...
		"settings.wire.why":         "How this program reaches the ports it opens. SOCKS5 carries UDP, so QUIC and resolving names at the far end work through it; an HTTP proxy speaks only TCP and those quietly do not happen. HTTP is here to fall back to.",
	...
	LangRU: {
		...
		"proxies.resting":        "Отдыхают после отказа",
		"proxies.ports":          "Портов открыто",
		"proxies.warm":           "Из них прогрето",
		"proxies.quarantined":    "Портов в карантине",
		"proxies.pool.why":       "Что верно в эту секунду, а не счёт произошедшего. Отказавший адрес уходит на отдых и потом возвращается; если отдыхающих становится почти столько же, сколько всего, перебирать скоро будет нечего.",
		...
		"proxies.kind.transport": "Запрос не дошёл — соединение отклонено, сброшено или закрыто",
		"proxies.kind.port":      "Порт самой прокси-службы не ответил — через него ничего не шло",
		"proxies.kind.relay":     "Отказ в передаче — адрес сам терминирует TLS",
		"proxies.kind.wall":      "Стена — источник отказал этой личности",
		"proxies.kind.timeout":   "Истёк наш собственный срок",
		"proxies.kind.other":     "Прочее",
		"proxies.reset":          "Обнулить счётчики",
		"proxies.release":        "Выпустить всех со скамейки",
		"proxies.reset.why":      "Ставит все счётчики выше на ноль, чтобы начать замер отсюда. Обнуляются именно счётчики, а не состояние: отдыхающий адрес продолжает отдыхать, порт в карантине остаётся в карантине.",
		"proxies.release.why":    "Возвращает в оборот все отдыхающие адреса и забывает записанные отдыхи. Нужно, когда скамейку наполнило не поведение адресов — перезапуск прокси-службы, пропавшая на минуту сеть, — и каждая запись на ней говорит об одном событии, а не о целом списке. Счётчики при этом не трогаются.",
		...
		"form.kind.why":          "Что каждый тип делает со строкой списка:",
		...
		"settings.wire.why":         "Как программа обращается к открытым портам. SOCKS5 переносит UDP, поэтому через него работают QUIC и разрешение имён на дальней стороне; HTTP-прокси умеет только TCP, и это просто не происходит. HTTP оставлен для отката.",
