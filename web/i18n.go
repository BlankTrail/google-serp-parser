...
var catalogue = map[Lang]map[string]string{
	LangEN: {
		...
		"job.reshape":          "Save the pool",
		"job.reshape.why":      "These three are read when the job next puts up a pool. A job that is running holds the one it raised for itself, so changing them here does not disturb it: what is saved is what it will come up on next time.",
		...
		"settings.search":           "Ports held for a search inside a request",
		"settings.search.count":     "The ports held for a search are a count, and none turns that address off.",
	...
	LangRU: {
		...
		"job.title":            "Задание",
		"job.progress":         "Ход",
		"job.reshape":          "Сохранить пул",
		"job.reshape.why":      "Эти три читаются, когда задание в следующий раз поднимает пул. Идущее задание держит тот, что подняло себе само, поэтому правка его не трогает: сохранённое — это то, на чём оно поднимется в следующий раз.",
		...
		"settings.search":           "Портов под поиск внутри запроса",
		"settings.search.count":     "Портов под поиск — число; ноль выключает эту точку входа.",
