...
var catalogue = map[Lang]map[string]string{
	...
	LangRU: {
		...
		"proxies.release.why":    "Снимает бан со всех отдыхающих адресов и забывает записанные отдыхи. Нужно, когда скамейку наполнило не поведение адресов — перезапуск прокси-службы, пропавшая на минуту сеть, — и каждая запись на ней говорит об одном событии, а не о целом списке. Счётчики при этом не трогаются.",
