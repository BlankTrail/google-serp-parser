...
var catalogue = map[Lang]map[string]string{
	LangEN: {
		...
		"proxies.gateways.unreachable": "The gateways could not be asked for: the connection in the settings did not answer.",
		...
		"settings.ban.why":            "How long an address is left out after it fails. A short ban brings the same dead addresses back inside one job; a long one takes more of the list out at once. Nought is the default of sixty.",
	...
	LangRU: {
		...
		"proxies.gateways.unreachable": "Не удалось запросить шлюзы: подключение из настроек не ответило.",
		...
		"settings.ban.why":            "Сколько адрес не выдаётся после отказа. Короткий бан возвращает те же мёртвые адреса внутри одного задания, длинный держит вне оборота большую часть списка. Ноль — умолчание в 60.",
