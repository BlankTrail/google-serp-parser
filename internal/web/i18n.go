...
var catalogue = map[Lang]map[string]string{
	LangEN: {
		...
		"job.checks.crowded":         "The checks are coming oftener than one in seven requests. That is the sessions being asked again before they have rested long enough — give them a longer rest on the job's settings.",
		"job.checks.why":             "A check passed is counted off the clearance Google leaves on the session that paid for it. Being solved and waiting are the whole service's, not this job's: the solver processes are licensed to the machine, and a job takes its turn among everything else asking of them.",
	...
	LangRU: {
		...
		"job.checks.crowded":         "Проверки приходят чаще одной на семь запросов. Это значит, что сессии спрашивают снова раньше, чем они отдохнули, — увеличьте отдых сессии в настройках задания.",
		"job.checks.why":             "Пройденная проверка считается по клиренсу, который Google оставляет на заплатившей за неё сессии. «Решается» и «ждут» — числа всей службы, а не задания: процессы решателя лицензированы на машину, и задание стоит в очереди за всеми, кто их просит.",
