...
var catalogue = map[Lang]map[string]string{
	LangEN: {
		...
		"form.identity.why":          "Left as they are, the ports are spread over every browser and system this program knows, at the newest ten releases of each the service holds. Three hundred ports that are all the newest Chrome on Windows are one identity three hundred times over. Naming a browser or a system narrows that spread to what was named; naming a browser, a system and a version runs the whole job on one identity.",
		...
		"form.wholepool.why":         "Ticked, this job opens a port of its own for every address the list can spare, as it asks for identities, and ports per thread means nothing: it spends the list rather than a few of it, and every request settles through a different address. What that costs is measured — a port on a fresh address pays for a challenge on its first request, one to three minutes against a second or two through one that has already answered, and on a large cheap list three addresses in four carry nothing at all.",
		...
		"form.pool.why":       "These four belong to the job and can be changed on its own page. Ports opened = threads × ports per thread, and as many again for reading the addresses a region hides — those are opened only as they are needed, and on a region that states its addresses, never.",
		...
		"proxies.vdns":                 "Where names are resolved",
		"proxies.vdns.auto":            "Through the exit where there is one",
		"proxies.vdns.on_leak":         "Only where resolving here would leak the name",
		"proxies.vdns.forced":          "Through the exit, always",
		"proxies.vdns.off":             "Here, whatever the traffic does",
		"proxies.vdns.unknown":         "That is not one of the four ways names can be resolved.",
		"proxies.solver":               "Challenge Breaker on these ports",
		"proxies.http3":                "Allow HTTP/3",
		"proxies.vdns.why":             "A port whose traffic leaves through an exit in one place while its names are resolved in another has told the far end two things about where it came from. The first setting resolves through the exit wherever the port has one to resolve through.",
		"proxies.solver.why":           "Google answers a challenged request with nothing a parser can read, and the solver is what carries the session through one. A tariff holds a fixed number of solver processes, so a profile carrying something that is never challenged is a profile worth turning it off on.",
		"proxies.http3.why":            "It applies where the exit can carry UDP, which a proxy from a list cannot: on such a profile the box changes nothing.",
	...
	LangRU: {
		...
		"form.identity.why":          "Если оставить как есть, порты разойдутся по всем браузерам и системам, которые знает программа, на десяти последних версиях каждого, что есть в сервисе. Триста портов, и все — последний Chrome на Windows, это одна личность триста раз. Указанный браузер или система сужают этот разброс до указанного; браузер, система и версия вместе — всё задание идёт на одной личности.",
		...
		"form.wholepool.why":         "С галкой задание открывает по порту на каждый адрес, который список может выделить, по мере того как ему нужны личности, а «портов на поток» ничего не значит: оно тратит список, а не несколько адресов из него, и каждый запрос уходит через свой. Цена этого измерена — порт на свежем адресе платит челленджем за первый запрос, от минуты до трёх против секунды-двух через тот, что уже отвечал, а на большом дешёвом списке три адреса из четырёх не везут ничего.",
		...
		"form.pool.why":       "Эти четыре — параметры задания, их можно изменить на его странице. Открывается портов = потоки × порты на поток, и столько же ещё на чтение адресов, которые регион прячет, — эти открываются только по мере надобности, а на регионе, который адреса печатает, не открываются вовсе.",
		...
		"proxies.vdns":                 "Где разрешаются имена",
		"proxies.vdns.auto":            "Через выход, где он есть",
		"proxies.vdns.on_leak":         "Только когда иначе имя утечёт",
		"proxies.vdns.forced":          "Через выход всегда",
		"proxies.vdns.off":             "Здесь, что бы ни делал трафик",
		"proxies.vdns.unknown":         "Это не один из четырёх способов разрешать имена.",
		"proxies.solver":               "Challenge Breaker на этих портах",
		"proxies.http3":                "Разрешить HTTP/3",
		"proxies.vdns.why":             "Порт, трафик которого уходит через выход в одном месте, а имена разрешает в другом, сообщил дальнему концу две разные вещи о том, откуда он. Первый вариант разрешает через выход везде, где выход есть.",
		"proxies.solver.why":           "На запрос с челленджем Google отвечает тем, что парсер прочитать не может, и через челлендж сессию проводит решатель. Процессов решателя в тарифе конечное число, так что профиль, через который идёт то, что челленджами не встречают, стоит от него освободить.",
		"proxies.http3.why":            "Работает там, где выход умеет UDP, а прокси из списка не умеет: на таком профиле галка ничего не меняет.",
