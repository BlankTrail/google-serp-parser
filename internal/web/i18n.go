...
var catalogue = map[Lang]map[string]string{
	LangEN: {
		...
		"job.reshape.total":   "Ports opened = threads × ports per thread, and as many again for reading the addresses a region hides — those are opened only as they are needed, and on a region that states its addresses, never.",
	...
	LangRU: {
		...
		"job.reshape.total":   "Открывается портов = потоки × порты на поток, и столько же ещё на чтение адресов, которые регион прячет, — эти открываются только по мере надобности, а на регионе, который адреса печатает, не открываются вовсе.",
