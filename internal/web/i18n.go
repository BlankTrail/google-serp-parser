...
var catalogue = map[Lang]map[string]string{
	LangEN: {
		...
		"job.checks.perthousand": "Captchas per 1000 pages",
		...
		"job.checks.why":         "A check passed is counted off the clearance Google leaves on the session that paid for it. Per 1000 pages is this run's checks for each thousand pages Google answered it, the checks a fresh session pays to be let in included. Being solved and waiting are the whole service's, not this job's: the solver processes are licensed to the machine, and a job takes its turn among everything else asking of them.",
	...
	LangRU: {
		...
		"job.checks.perthousand": "Каптч на 1000 страниц",
		...
		"job.checks.why":         "Пройденная проверка считается по клиренсу, который Google оставляет на заплатившей за неё сессии. «На 1000 страниц» — проверки этого прогона на каждую тысячу страниц, которые ответил Google, включая проверки, которыми свежие сессии платят за вход. «Решается» и «ждут» — числа всей службы, а не задания: процессы решателя лицензированы на машину, и задание стоит в очереди за всеми, кто их просит.",
