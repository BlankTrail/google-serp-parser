...
var catalogue = map[Lang]map[string]string{
	LangEN: {
		...
		"form.wholepool":             "Use the whole proxy list",
		"form.wholepool.why":         "A port of its own for every address the list can spare, opened as identities are asked for; ports per thread then means nothing. Every request goes out through a different address. The cost is measured: the first request on a fresh address pays for a challenge — one to three minutes against a second or two through one that has already answered.",
		...
		"job.reshape.total":   "Ports opened = threads × ports per thread, and as many again for reading the addresses a region hides — those only as they are needed.",
		...
		"form.pool.why":       "These four can also be changed on the job's own page. Ports opened = threads × ports per thread, and as many again for reading the addresses a region hides — those only as they are needed.",
		...
		"proxies.hop":                  "First hop",
		"proxies.hop.none":             "None — straight to the address",
		"proxies.hop.socks5":           "SOCKS5 proxy",
		"proxies.hop.gateway":          "BlankTrail gateway",
		"proxies.hop.proxy":            "First hop proxy",
		"proxies.hop.gateway.pick":     "First hop gateway",
		"proxies.hop.why":              "The road the ports take to their addresses. Addresses this machine reaches badly answer through a first hop: on a wingate list, straight from here the solver could not reach Google through a single address, and through a first hop 46 searches in 48 were answered. The address stays the exit Google sees.",
		"proxies.hop.needs.proxy":      "Name the SOCKS5 proxy the ports go through first.",
		"proxies.hop.proxy.bad":        "A first hop is a SOCKS5 proxy: socks5://, the host and the port, and the login and password if it takes them.",
		"proxies.hop.needs.gateway":    "Choose the gateway the ports go through first.",
		"proxies.hop.unknown":          "That is not a kind of first hop this program knows.",
		...
		"settings.renew.why":           "How often a port is opened again, with a fresh fingerprint and an empty cookie jar. Nought never does; a short list of gateways wants about ten minutes.",
	...
	LangRU: {
		...
		"form.wholepool":             "Использовать весь пул прокси",
		"form.wholepool.why":         "По порту на каждый адрес, который может выделить список, по мере того как нужны личности; «портов на поток» при этом ничего не значит. Каждый запрос уходит через свой адрес. Цена измерена: первый запрос на свежем адресе платит челленджем — от минуты до трёх против секунды-двух через тот, что уже отвечал.",
		...
		"job.reshape.total":   "Открывается портов = потоки × порты на поток, и столько же на чтение адресов, которые регион прячет, — эти только по мере надобности.",
		...
		"form.pool.why":       "Эти четыре меняются и на странице задания. Портов открывается = потоки × порты на поток, и столько же на чтение адресов, которые регион прячет, — эти только по мере надобности.",
		...
		"proxies.hop":                  "Первый хоп",
		"proxies.hop.none":             "Нет — прямо к адресу",
		"proxies.hop.socks5":           "SOCKS5-прокси",
		"proxies.hop.gateway":          "Шлюз BlankTrail",
		"proxies.hop.proxy":            "Прокси первого хопа",
		"proxies.hop.gateway.pick":     "Шлюз первого хопа",
		"proxies.hop.why":              "Дорога, по которой порты идут к своим адресам. Адреса, до которых отсюда плохой маршрут, отвечают через первый хоп: на списке wingate напрямую решатель не смог дойти до Google ни через один адрес, а через первый хоп ответили 46 поисков из 48. Выходом, который видит Google, остаётся адрес.",
		"proxies.hop.needs.proxy":      "Укажите SOCKS5-прокси, через который порты идут первым хопом.",
		"proxies.hop.proxy.bad":        "Первый хоп — это SOCKS5-прокси: socks5://, хост и порт, логин и пароль, если он их просит.",
		"proxies.hop.needs.gateway":    "Выберите шлюз, через который порты идут первым хопом.",
		"proxies.hop.unknown":          "Такого вида первого хопа программа не знает.",
		...
		"settings.renew.why":           "Как часто порт открывается заново — со свежим отпечатком и пустыми куками. Ноль — никогда; короткому списку шлюзов подходит около десяти минут.",
