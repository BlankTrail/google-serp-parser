...
func TestSupervisor_TellsThePoolWhenAJobSpendsTheWholeList(t *testing.T) {
	// The tick is written down with the job and read back when its identities
	// are raised. A supervisor that dropped it would start the run on a pool of
	// a fixed size, and nothing on any screen would say which of the two the
	// job was actually on.
	st := testStore(t)
	if _, err := st.CreateJob(t.Context(),
		store.JobSpec{Name: "whole", Pages: 1, Threads: 3, Ports: 5, WholePool: true},
		[]string{"a"}); err != nil {
