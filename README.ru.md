...
> ⚠️ **Программа работает только через [BlankTrail Proxy](https://blanktrail.com) с [Challenge Breaker](https://blanktrail.com/#features).**
...
> **[Challenge Breaker](https://blanktrail.com/#features)** — жёсткое требование, а не рекомендация.
>
> **[Challenge Breaker](https://blanktrail.com/#features) проходит reCAPTCHA.** Посчитано на живом прогоне: 721
...
**1. Установите и запустите [BlankTrail Proxy](https://blanktrail.com)** с лицензией, включающей
[Challenge Breaker](https://blanktrail.com/#features). Запомните адрес его control API (по умолчанию
...
**5. Откройте «Прокси»** и укажите, откуда брать выходы. Источников три, и всё,
что с ними связано, живёт на этом же экране:

- **файл на этой машине** — по одному `host:port` в строке, с логином и паролем
  или без;
- **адрес** — тот же список, забираемый у провайдера и перечитываемый с заданным
  интервалом;
- **VPN-шлюзы из BlankTrail** — конфигурации вашей подписки, разложенные по
  подпискам; отметить можно одну, целую подписку или все сразу, и рядом с каждой
  виден последний замеренный отклик.

Там же задаётся, на сколько выводится из оборота отказавший адрес, сколько
потоков делят один адрес или шлюз, по SOCKS5 или HTTP открывать порты и как
часто порт меняет личность. И там же читается состояние пула во время работы:
сколько адресов в бане, сколько портов открыто и прогрето, что и по какой
причине отказало.

**6. Откройте «Новое задание»**, вставьте фразы или загрузите `.txt` и выберите,
что с ними делать: собрать выдачу, проверить позиции сайта или проверить, есть
ли адрес в индексе Google. Дальше страна, глубина и число потоков. Форма скажет,
во что обойдётся прогон, ещё до запуска.
...
Интерфейс говорит по-русски и по-английски. Язык выбирается на экране
«Настройки» — там же, где подключение, прогретые личности и доступ по
сети.
