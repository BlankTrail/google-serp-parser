...
- **Редирект, которого ждёт разворот адреса, — это ответ, а не стена.** Скрытый
  адрес читается из заголовка `Location` редиректа, поэтому каждый удачный
  разворот отвечает 302 — а 302 это то, как выглядит блок-страница Google. Пул
  засчитывал его против порта так же: три разворота подряд выбрасывали из порта
  тот самый адрес, который их и вёз, и следующие поиски уходили через то, что
  список подсунет дальше. На регионе, который скрывает адреса, это большая часть
  всех запросов задания — прогон разбирал собственный пул быстрее, чем собирал
  его. Теперь вызывающая сторона говорит, какой ответ она просила. Поиск не
  изменился: у него редирект никто не просил, и он по-прежнему считается стеной.
