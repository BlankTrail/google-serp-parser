...
У списка — файла или адреса — есть ещё **Первый хоп**: дорога, по которой его
порты идут к адресам. «Нет» ведёт прямо к каждому адресу. «SOCKS5-прокси»
пускает каждый порт сначала через этот прокси; он записывается так же, как адрес
в списке, с логином и паролем. «Шлюз BlankTrail» пускает их через один из шлюзов
сервиса, выбранный из его списка. Это для списка, до которого с этой машины
плохой маршрут: на списке wingate напрямую решатель проверок не смог открыть
соединение ни через один адрес, и каждый поиск возвращался JavaScript-проверкой
Google, а через первый хоп ответили 46 поисков из 48. Выходом, который видит
Google, остаётся адрес, поэтому сессия на нём остаётся той же сессией. У профиля
на шлюзах первого хопа нет — дорога шлюза задаётся на нём самом в BlankTrail.
