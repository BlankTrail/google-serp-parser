...
| Портов на поток | Личностей на поток. Потоки × порты — размер пула. По умолчанию одна: поток отрабатывает паузу одной личности на других своих проводках, поэтому вторая не даёт перекрытия, а каждая личность в работе — это ещё одна, которую придётся вести лицензированному решателю челленджей |
| Использовать весь пул прокси | По порту на каждый адрес, который список может выделить, по мере того как прогону нужны личности, пока не кончится список или место на сервисе. «Портов на поток» при этом ничего не значит |
...
- Там же проверки Google: сколько задание прошло, сколько решается, сколько ждёт
  решателя и сколько запросов приходится на одну проверку. Последнее считается
  по сессиям, которым Google уже отвечал с того же адреса: сессия, которую
  впускают, и сессия, попавшая на новый адрес, платят проверкой при любой
  скорости. Если разгон уже закончен, а проверки идут чаще одной на семь таких
  запросов, значит сессии спрашивают снова раньше, чем они отдохнули, — страница
  об этом говорит.
...
| Портов на поток | Личностей на поток. Потоки × порты = размер пула. По умолчанию одна — почему, сказано в таблице настроек выше |
| Использовать весь пул прокси | По порту на каждый адрес, который список может выделить, пока не кончится список или место на сервисе. «Портов на поток» при этом ничего не значит |
