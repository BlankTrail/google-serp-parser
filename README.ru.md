...
*Небольшой демо-пул на двадцать портов по изрядно поработавшему списку. Доля
ошибок здесь — то, что этот список даёт на таком размере; цифры в разделе
[Производительность](#-производительность) сняты на трёхстах прогретых
личностях.*
...
**2. Возьмите gserp.** Либо скачайте сборку из релизов, либо соберите сами:

```
go build -o gserp.exe ./cmd/gserp
```

**3. Запустите `start.bat`.** Он поднимет программу и откроет
`http://127.0.0.1:8080` в браузере. Если `gserp.exe` рядом нет, но установлен
Go, сборка выполнится сама.
