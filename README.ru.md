...
[релизов](https://github.com/BlankTrail/google-serp-parser/releases/latest) и
распакуйте куда угодно:

| Система | Файл |
|---|---|
| Windows, обычный ПК | `gserp-<версия>-windows-amd64.zip` |
| Windows на ARM | `gserp-<версия>-windows-arm64.zip` |
...
В каждом архиве — программа, файл запуска для этой системы, оба README и
лицензия. Программа состоит из одного файла, ничего доустанавливать не нужно, а
историю она держит в `gserp.db` рядом с собой.

**3. Запустите.**

- **Windows** — двойной щелчок по `start.bat`.
- **Linux и macOS** — `./start.sh` в терминале.

В обоих случаях программа поднимется и откроет браузер на
`http://127.0.0.1:8080`. Слушает она только эту машину.
...
> `xattr -d com.apple.quarantine gserp` в распакованной папке.
