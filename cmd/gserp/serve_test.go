...
func TestServe_OpensThePortsAConnectionSavedInTheBrowserDescribes(t *testing.T) {
	...
	// The settings reach the ports through a profile now, and carrying them into
	// one is what the interface does at startup before it raises anything. A
	// test that skipped it would be asking about a machine with no profiles,
	// which is a machine that has never been started.
	saved, _ := opts.saved(io.Discard)
	if _, err := st.CarryProxySettings(t.Context(), theFirstProfile(saved, true)); err != nil {
		t.Fatalf("carrying the settings into a profile: %v", err)
	}
...
func TestServe_CarriesTheSavedSourceWholeToTheListItReads(t *testing.T) {
	...
	for _, saved := range []store.Profile{
