...
//
// It sits beside the browser interface rather than on top of it. Neither one
// imports the other; the command that starts the process mounts both against
// the same history, so a job set up by a program and a job set up by hand are
// the same job.
//
